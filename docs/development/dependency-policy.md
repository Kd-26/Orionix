...
Future extras may be introduced for `gpu`, `vllm`, `sglang`, `runpod`, `modal`, `jarvislabs`, `control-plane`, and `telemetry`; `dev` and test tooling currently use the root dependency group. Empty extras are not published merely to advertise intent. Provider/GPU extras must never be required by the base CPU installation.
